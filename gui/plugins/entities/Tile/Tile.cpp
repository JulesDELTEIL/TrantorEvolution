...
Tile::Tile(const sf::Vector2f& pos, BiomeTypes_e type) : AEntity(pos)
{
    try {
        ...
        _drawables["background"] = ecs::ECSFactory::create<ecs::IDrawable>("biome", pos.x, pos.y, type);
        if (type == GRASS)
            _drawables["tree"] = ecs::ECSFactory::create<ecs::IDrawable>("resource_node", pos.x + 8, pos.y + 8, type);
